...
레이아웃 수식은 프로토타입("generate ppt.py")에서 이미 확정된 사양이므로 그대로 이식한다.
build_presentation_from_job()만 backend/models/schema.py의 PPTJob을 받는 어댑터로 새로 추가됨.
"""
import re
...
def build_presentation_from_job(job, photo_sets) -> str:
    """backend/models/schema.py의 PPTJob을 받아 build_presentation()을 호출하는 얇은 어댑터.

    photo_sets는 {compos_id: [{before, after, before_date, after_date}, ...]} 형태로
    services/generate_service.py가 SessionState의 start/mid/end 사진을 그룹핑해 만들어 전달한다
    (구도별 전후 페어링에는 세션 상태가 필요해 여기서는 조립하지 않는다 — generate_ppt.py는
    순수 렌더링 라이브러리로 유지).
    """
    body_comp_rows = [asdict(r) for r in job.body_comp_rows]
    return build_presentation(job.patient.name, job.mode, photo_sets, body_comp_rows, job.output_path)
